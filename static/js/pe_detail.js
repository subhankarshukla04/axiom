...
function getUserValuation(cid) {
    return JSON.parse(localStorage.getItem('userPEValuations') || '{}')[cid] || null;
}
...
function saveUserValuation() {
    ...
    const store = JSON.parse(localStorage.getItem('userPEValuations') || '{}');
    store[currentCompanyId] = { impliedMarketCap, pricePerShare, peMultiple: multiple, savedAt: new Date().toISOString() };
    localStorage.setItem('userPEValuations', JSON.stringify(store));
...
function removeUserValuation() {
    ...
    localStorage.setItem('userPEValuations', JSON.stringify(store));
    resetSensitivity();
    document.getElementById('pe-remove-valuation-btn').style.display  = 'none';
    document.getElementById('pe-user-valuation-indicator').style.display = 'none';
    document.getElementById('pe-save-valuation-btn').innerHTML = 'Save Your Valuation';
}
...
function resetSensitivity() {
    ...
    const b = peBaseInputs;
    document.getElementById('pe-sens-multiple').value       = b.pe_multiple;
    document.getElementById('pe-sens-multiple-input').value = b.pe_multiple.toFixed(1);
    recalculate();
}
...
function recalculate() {
    ...
    const axiomMultiple = parseFloat(base.pe_multiple.toFixed(1));
    const axiomPrice = (base.net_income * axiomMultiple) / base.shares;
...
function renderPEDetails(pe) {
    ...
    const peMultipleTooltip = `P/E Multiple: ${assumptions.pe_multiple?.value?.toFixed(1)}x\n\n${peMultipleNote}\n\nReflects what investors typically pay per dollar of earnings. Higher multiples indicate expectations of stronger future growth.`;
    ...
    <div style="padding:24px;border-bottom:1px solid var(--border-color);">
        ...
        <div style="background:var(--bg-primary);border:1px solid var(--border-color);border-radius:8px;padding:20px;display:flex;align-items:center;gap:20px;">
            <div style="flex:1;">
                ...
                <div style="font-size:1.8rem;font-weight:700;color:var(--text-primary);">${assumptions.pe_multiple?.value?.toFixed(1)}x</div>
            </div>
            <div style="flex:2;padding-left:20px;border-left:1px solid var(--border-color);">
                <div style="font-size:0.85rem;color:var(--text-secondary);line-height:1.5;">
                    ${peMultipleNote}. Reflects what investors typically pay per dollar of earnings. Higher multiples indicate expectations of stronger future growth.
                </div>
            </div>
        </div>
    </div>`;
        ...
        <div style="background:var(--bg-primary);border:1px solid var(--border-color);border-radius:8px;padding:32px;">
            <div style="display:flex;align-items:center;justify-content:center;gap:20px;flex-wrap:wrap;font-size:1.5rem;">
                <a href="${baseUrl}/financials" target="_blank" rel="noopener"
                   ...
                   style="font-weight:700;color:var(--accent-primary);text-decoration:none;border-bottom:2px dotted var(--accent-primary);padding-bottom:2px;">${fmt(inputs.net_income?.value)}</a>
                <span style="color:var(--text-muted);font-size:1.6rem;">×</span>
                <span title="${peMultipleTooltip}"
                      style="font-weight:700;color:var(--text-primary);cursor:help;border-bottom:2px dotted var(--text-muted);padding-bottom:2px;">${assumptions.pe_multiple?.value?.toFixed(1)}x</span>
                <span style="color:var(--text-muted);font-size:1.6rem;">=</span>
                <span title="Implied Market Cap\nComputed from Net Income × P/E Multiple"
                      style="font-weight:700;color:var(--text-primary);cursor:help;border-bottom:2px dotted var(--text-muted);padding-bottom:2px;">${fmt(calculated.implied_market_cap?.value)}</span>
            </div>
    ...
    <div style="padding:24px;border-bottom:1px solid var(--border-color);">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;align-items:center;">
            <div style="padding:20px;background:var(--bg-primary);border:1px solid var(--border-color);border-radius:8px;">
                ...
                <input type="range" id="pe-sens-multiple" min="5" max="50" step="0.1"
                       value="${base.pe_multiple || 20}"
                ...
                <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:12px;">
                    <input type="number" id="pe-sens-multiple-input" step="0.1"
                           value="${(base.pe_multiple || 20).toFixed(1)}"
    ...
    <div style="padding:24px;">
        <div id="pe-user-valuation-indicator" style="display:${hasSaved ? 'block' : 'none'};margin-bottom:12px;padding:10px 14px;background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.3);border-radius:6px;font-size:0.85rem;color:var(--text-secondary);">
        ...
        <div style="display:flex;gap:10px;">
            <button id="pe-save-valuation-btn" onclick="saveUserValuation()"
            ...
            <button id="pe-remove-valuation-btn" onclick="removeUserValuation()"
...
function initTooltips() {
    ...
    const content = document.getElementById('pe-page-content');
...
async function init() {
    ...
    const content = document.getElementById('pe-page-content');
    content.innerHTML = '<div style="padding:60px;text-align:center;color:var(--text-muted);font-size:1rem;">Loading P/E breakdown…</div>';
    ...
    try {
        ...
        if (data.company_name) {
            document.title = `${data.company_name} — P/E | AXIOM`;
        ...
        document.getElementById('hdr-fv').textContent     = s.pe_fv         != null ? '$' + s.pe_fv.toFixed(2)          : (s.fair_value != null ? '$' + s.fair_value.toFixed(2) : '—');
        ...
        if (data.pe_details) {
            peDetailsCache = data.pe_details;
            peBaseInputs   = data.pe_details.base_inputs || null;
            content.innerHTML = renderPEDetails(data.pe_details);
            initTooltips();
        } else {
            content.innerHTML = '<div style="padding:60px;text-align:center;color:var(--text-muted);">P/E details not available for this company.</div>';
        }
    } catch (err) {
